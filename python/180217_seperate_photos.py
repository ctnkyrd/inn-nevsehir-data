...
#                 if(photo.split('_')[1].decode('utf-8')!=u"Taskinpasa"):
...
for r,d,f in os.walk(photo_directory):
    for photo in f:
        if photo.endswith('jpg'):
            try:
                src = os.path.join(r,photo.replace('Taskinpasa','Taşkınpaşa').decode('utf-8'))
                if(photo.split('_')[1].decode('utf-8') == u"Taskinpasa"):
                    dest = new_directory +"\\"+photo.split('_')[2]+"_"+photo.split('_')[3]+".jpg"
                    copyfile(src, dest)
                    print photo 
            except BaseException as be:
                print be.message
